...
function subscribeConst() {
	constellation.client.registerStateObjectLink("*", "SfitDataManager", "TodaySteps"+username, "*", (so) => {
		let tmp = JSON.parse(so.Value);
		if (element && distancePrint && calBurnPrint && objectiveStep) {
			console.log(so);
			element.innerHTML = tmp.step;
			objectiveStep.innerHTML = tmp.goal;
			distancePrint.innerHTML = (calculDistance(userSize, tmp.step)/100).toFixed(2) + " m";
			calBurnPrint.innerHTML = (calculBurnCal(tmp.step)).toFixed(2) + " Cal";

			goBar(tmp.step, tmp.goal);
		}
	});
}
